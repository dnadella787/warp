...
net::http::request to_request(const beast_request &req) {
    ...
    net::http::request warp_req(
	map_method(req.method()), std::move(target), req.body(), std::move(hdrs));
    warp_req.set_path(std::move(path));
    warp_req.set_query_params(std::move(query_params));
    return warp_req;
}

std::shared_ptr<beast_response> to_beast_response(
    const net::http::response &resp, const beast_request &req) {
...
void session::write_response(net::http::response resp) {
	...
	[self = shared_from_this(), be_resp, close](boost::beast::error_code ec,
	    std::size_t bytes_transferred) { self->on_write(ec, bytes_transferred, close); });
}
