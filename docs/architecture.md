...
- `net/http.hpp`: Defines request/response wrappers, HTTP method enums, query/path parameter helpers, and error translation helpers.
...
3. Boost.Beast parses incoming bytes into HTTP requests, which are translated into Warp's value-based `request` (including parsed query parameters and lazy JSON helpers).
